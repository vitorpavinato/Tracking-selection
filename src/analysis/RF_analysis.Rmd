...
## Simulation parameter´s inference
...
randompods_genload_table <- data.frame(est=posterior_radnompods_averageGenLoad$expectation,
                                       ...
                                       selection = ifelse(pooled_reftable_pods[,"POPPrStrongMSel"] == 0, "qNeutral", "sSel"),
...
ggplot(randompods_genload_table, aes(x=obs , y=est, color=selection)) + 
...
ggplot(randompods_genload_table, aes(x=obs_2 , y=est_2, color=selection)) + 
  geom_point(size=3, alpha=0.75) + 
  scale_colour_manual(values = c("#74add1","#f46d43"),
                      name   = "Classification",
                      breaks = c("qNeutral", "sSel"),
...
legend("bottomleft", legend = c("neutral","mutation limited", "mutation unlimited"), 
...
ggplot(log_randompods_thetaS_table, aes(x=obs , y=est, color=selection)) + 
  geom_point(size=3, alpha=0.75) + 
  scale_colour_manual(values = c("#74add1","#f46d43"),
                    name   = "Classification",
                    breaks = c("qNeutral", "sSel"),
...
randompods_popstrongmsel_table <- data.frame(est_1=posterior_radnompods_logpopstrongmsel$expectation[-qneutral_qneutral],
                                             ...
                                             classification=ifelse(class_selection_1_table_sSel$obs == "qNeutral" & class_selection_1_table_sSel$pred == "sSel", "a",
...
ggplot(randompods_popstrongmsel_table, aes(x=obs_1 , y=est_1, color=classification)) + 
  geom_point(size=3, alpha=0.75) + 
...
ggplot(randompods_popstrongmsel_table, aes(x=obs_2 , y=est_2, color=classification)) + 
  geom_point(size=3, alpha=0.75) + 
  scale_colour_manual(values = c("#a50026","#f46d43","#006837"),
                      name   = "True-Predicted",
                      breaks = c("a","b","c"),
...
hist(log_radnompods_meanNe2, freq = TRUE, xlab = expression(log[10](italic(bar(N[e[(2)]])))), main = "", col = "#bdbdbd")
...
## boxplot
fixedpods_meanNe2_data <- data.frame(model  = c(rep("A", 300), rep("B", 300), rep("C", 300)), 
                                     type   = c(rep("a",100), rep("b",100), rep("c",100), 
                                                rep("a",100), rep("b",100), rep("c",100),
                                                rep("a",100), rep("b",100), rep("c",100)),
                                     values = c(log_fixedpods_meanNe2[1:100], posterior_fixedpods_logmeanNe2$expectation[1:100], log10(wfabc_raw_ne2_neutral$wfabcne/2),
                                                log_fixedpods_meanNe2[101:200], posterior_fixedpods_logmeanNe2$expectation[101:200], log10(wfabc_raw_ne2_limited$wfabcne/2),
                                                log_fixedpods_meanNe2[201:300], posterior_fixedpods_logmeanNe2$expectation[201:300], log10(wfabc_raw_ne2_unlimited$wfabcne/2)))
...
boxplot(fixedpods_meanNe2_data$values ~ fixedpods_meanNe2_data$type * fixedpods_meanNe2_data$model,
        ...
        cex.axis = 1,
        cex.lab  = 1,
        col = c(rep("#1f78b4",3),rep("#ff7f00",3),rep("#e31a1c",3)))
...
text(x = 1:9, 
     ...
     adj=1, 
     xpd=TRUE, 
     labels = paste(rep(c("True", "ABC-RF", "WFABC"),3)),
     cex = 1)
```
...
boxplot(fixedpods_ncs_data$values ~ fixedpods_ncs_data$type * fixedpods_ncs_data$model,
        lwd = 2,
        ylim = c(0,3),
        ylab = expression(log[10](hat(italic(N[cs])))),
        xlab = "",
        xaxt = "n",
        cex.axis = 1,
        cex.lab  = 1,
