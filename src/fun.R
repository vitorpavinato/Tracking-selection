...
do_sim <- function(sim, nsim, 
  ...
  if (file.exists(paste0(slim_output_folder, "slim_coalesced_",model_title,"_", sim, ".tree"))){
    ...
    filenames_genome <- paste0(slim_output_folder, "slim_output_paaf_t", seq(from=0, to=tau, by=1), "_", sim, ".txt")
    filenames_extraChr <- paste0(slim_output_folder, "slim_output_paaf_extraChr_t", seq(from=0, to=tau, by=1), "_", sim, ".txt")
      
    if(all(file.exists(c(filenames_genome, filenames_extraChr, slim_output_lastgen)))){
      ...
      if(!is.null(all_merged_genome)){
        ...
        ## IBD AND VAR NE - ALL MUTATION TYPES
        ...
        ## IBD AND VAR NE - ONLY NEUTRAL MUTATIONS
      ...
      if(!is.null(all_merged_extraChr)){
        ...
        merged_extraChr <- all_merged_extraChr[which(all_merged_extraChr$GO <= lastgen), ]
      ...
      ## IBD AND VAR NE - NEUTRAL IN EXTRA CHROMOSOME
      IBDNeChrtimes <- as.data.frame(t(rep(NA, tau)))
      colnames(IBDNeChrtimes) <- paste0("IBDNeChr",seq(from=0,to=(tau-1)),"_",seq(from=1, to=tau))
      ...
      if (debug_sim){
        ...
        file.copy(from = paste0(slim_output_folder, "slim_coalesced_",model_title,"_", sim, ".tree"), to = debug_output_folder)
        file.copy(from = slim_output_lastgen, to = debug_output_folder)
        file.copy(from = filenames_genome, to = debug_output_folder)
        file.copy(from = filenames_extraChr, to = debug_output_folder)
        
        debug_message <- "no PAAF file found"
    ...
    if (remove_files){
      ...
      file.remove(slim_output_lastgen)
      file.remove(filenames_genome)
      file.remove(filenames_extraChr)
