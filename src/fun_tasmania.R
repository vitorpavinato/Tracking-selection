...
do_sim <- function(sim, nsim, 
                   ...
                   missing_data, haplotype, SS1, SS2, tau,                                    # add radseq_sampled and radseq_interval and one_snp_radseq
  ...
  if (file.exists(paste0(slim_output_folder, "slim_coalesced_",model_title,"_", sim, ".tree"))){
    ...
    if(all(file.exists(c(slim_output_sample_t1, slim_output_sample_t2)))){
      ...
      if (genomeS > 2^29){
        ...
        system(paste(path_to_tabix, "-C", paste0(slim_output_sample_t1_sorted, ".gz"))) 
        system(paste(path_to_tabix, "-C", paste0(slim_output_sample_t2_sorted, ".gz")))
      ...
      if(file.exists(slim_output_sample_merged)){
        ...
        if (data_type == 2){
          ...
          slim_raw_data <- slim_raw_data[which(slim_raw_data$position %in% radseq_interval), ]
          
          slim_raw_data$chrom <- sapply(slim_raw_data$position, radseqtagging, tagSampled=radseq_sampled, readLength=radseq_readL)
          
          if (one_snp_radseq){
            slim_raw_data <- slim_raw_data[!duplicated(slim_raw_data[ ,1]), ]
          }
        }
        ...
        if (nrow(slim_raw_data) != 0){
          ...
          slim_snp_geno[sample(1:length(slim_snp_geno), size=round(length(slim_snp_geno)*missing_data), replace = FALSE)] <- NA
          ...
          count_ref_geno    <- apply(slim_snp_geno, 1, function(x){all(x == "11" | x == "00")})
          count_alt_geno    <- apply(slim_snp_geno, 1, function(x){all(x == "22" | x == "00")})
          keep_snps         <- !(count_ref_geno | count_alt_geno) # MARK MONOMORPHIC MUTATIONS
